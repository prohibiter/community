...
public class UserService implements CommunityConstant {
    ...
    public Map<String, Object> register(User user) {
        ...
        // 判断账号是否已经存在
        User has = userMapper.selectByName(user.getUsername());
        if (has != null) {
            map.put("usernameMsg", "账号已经存在");
            return map;
        }
        User userMail = userMapper.selectByEmail(user.getEmail());
        if (userMail != null) {
            map.put("emailMsg", "邮箱已经使用");
            return map;
        }
        // 注册用户
    ...
    public int updateHeader(int userId, String headerUrl) {
        ...
        final int rows = userMapper.updateHeader(userId, headerUrl);
        clearCache(userId);
        return rows;
    }
